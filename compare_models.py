...
def compare_plots():
    ...
    for i, model_dir in enumerate(model_dirs):
        ...
        axes[i].imshow(img)
        axes[i].set_title(model_dir)
        axes[i].set_xticks([])
        axes[i].set_yticks([])

        if i % 4 == 0:
            axes[i].set_ylabel(size_labels[model_dir.split('_')[0]], fontsize=10)
        

    fig.suptitle('Model Comparison: Training Loss & Training/Testing Accuracy', y=0.93, fontsize=14)

    fig.text(0.07, 0.5, '1st Hidden Layer Size', va='center', rotation='vertical', fontsize=14)
...
def main():
    global_best, best_epochs = best_epochs_accuracy()
